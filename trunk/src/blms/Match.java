 ...
 * A match result can be entered with the following required info: date, winning player, losing player.
 * The following optional information can also be informed: race length (number of balls to be made by highest-ranked player), the final score, the match's longest run for each player
 */
public class Match implements Comparable<Match> {
	 ...
	 */
	Match(League league, Date date, User winner, User loser) throws BlmsException {
		this(league, date, winner, loser, UNDEFINED, UNDEFINED, UNDEFINED, UNDEFINED);
	}

	/**
	 * Creates a match result. Can't be invoked directly. A match
	 * should be created by the method 
	 * {@link Registry#addMatchResult(League, Date, User, User, int, int, int, int)}.
	 * Some parameters can assume the value {@link Match#UNDEFINED}, meaning that the
	 * value is not known.
	 * @param league the league to which this match is part.
	 * @param date the date of the match.
	 * @param winner the player who won the match.
	 * @param loser the player who lost the match.
	 * @param length number of balls to be made by highest-ranked player or {@link Match#UNDEFINED}.
	 ...
	 * @throws BlmsException if (i) winner and loser are the same player, or 
	 * (ii) score is invalid (see {@link Match#setScore(int)}, or 
	 * (iii) losers's longest run is invalid (see {@link Match#setLongestRunForLoser(int)}), or 
	 * (iv) winner's longest run is invalid (see {@link Match#setLongestRunForWinner(int)}).
	 */
	 ...
	 * 
	 * @param score
	 * @throws BlmsException if score < 0 or score >= the match's length (number of balls to be made by highest-ranked player).
	 */
	 ...
	 * @throws BlmsException unless 1 <= longestRunForWinner <= length. 
	 */
	 ...
	 * @throws BlmsException unless 0 <= longestRunForLoser <= score.
	 */
